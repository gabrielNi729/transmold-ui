<template>
    <div class="outer" @mouseenter="showSwitch" @mouseleave="hideSwitch" :class="[onSwitch? 'long' : 'short']">
        ...
        <select v-else-if="curInputType === 'variable'" v-model="value" :class="{'left': !onSwitch}">
            <option v-for="i in variables" :value="i">{{i.getLabel()}}</option>
        </select>
...
<script>
    ...
    const allInputTypes = [Types.INPUT.CONSTANT, Types.INPUT.VARIABLE, Types.INPUT.INSTANCE];
    ...
    export default {
        ...
        props: ['init', 'variables', 'instances'],
        ...
        mounted(){
            if(this.init !== null && this.init !== undefined){
                this.inputIndex = this.inputTypes.indexOf(Types.identifyInputType(this.init));
                this.doEmit = false;
